...
class LogCapture:
    """
    Wraps sys.stdout so every print() call is forwarded to the browser
    overlay's live activity log via chat_server.push_log(), while still
    printing normally to the real terminal.
    """
    LEVEL_MAP = {
        "✅": "OK",   "ℹ️": "INFO", "⚠️": "WARN",
        "⏩": "SKIP", "🔹": "STEP", "❌": "FAIL",
    ...
    def write(self, text):
        ...
        if line:
            level = "INFO"
            for icon, lvl in self.LEVEL_MAP.items():
                if icon in line:
                    level = lvl
                    break
            try:
                _chat_server.push_log(line, level)
            except Exception:
                pass
...
if __name__ == '__main__':
    ...
    # Mirror all terminal output to the browser overlay's live log
    sys.stdout = LogCapture(sys.stdout)

    bot = LinkedinEasyApply(parameters, browser)
    bot.browser_factory = init_browser  # Allow the bot to restart the browser after daily limit
    bot.login()
    bot.security_check()
    bot.start_applying()